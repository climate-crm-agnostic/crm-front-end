...
import {
    sendContractMessage, getContractConversations, getContractConversation,
    renameContractConversation, deleteContractConversation,
} from "../services/contractAiService";
...
// duplicated rather than extracted since it's UI-only and each page owns its
// own rendering; not worth a shared module for this small a function.
    ...
    <div className="flex gap-1 items-center px-1 py-0.5">
        {[0, 1, 2].map(i => (
            <span key={i} style={{
                ...
                animation: "contract-ai-dot 1.2s infinite",
...
export const ContractAIChat = () => {
    ...
    const openConversation = useCallback(async (id) => {
        if (id === convId) return;
        try {
            const data = await getContractConversation(id);
            setConvId(data.id);
    ...
    const commitRename = async (id) => {
        ...
        try {
            await renameContractConversation(id, renameVal.trim());
    ...
    const handleDelete = async (id, e) => {
        e.stopPropagation();
        try {
            await deleteContractConversation(id);
    ...
    return (
        <>
            ...
            <div style={{ display: "flex", margin: "-1rem", marginTop: 0, height: "calc(100vh - 3rem)", overflow: "hidden" }}>

                {/* ── Left sidebar ─────────────────────────────────────── */}
                <div style={{ width: 260, flexShrink: 0, borderRight: "1px solid var(--border)", background: "var(--card)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
                    <div style={{ padding: "14px 16px 10px", borderBottom: "1px solid var(--border)" }}>
                        <span style={{ fontWeight: 700, fontSize: 15 }}>Contract AI</span>
                    </div>
                    ...
                    <div style={{ flex: 1, overflowY: "auto", padding: "4px 8px 12px" }}>
                        {convLoading && (
                            <p style={{ fontSize: 12, color: "var(--muted-foreground)", textAlign: "center", padding: "20px 0" }}>Loading...</p>
                        )}
                        {!convLoading && conversations.length === 0 && (
                            <p style={{ fontSize: 12, color: "var(--muted-foreground)", textAlign: "center", padding: "20px 0" }}>No conversations yet.</p>
                        )}
                                    ...
                                    >
                                        {renamingId === conv.id ? (
                                            <input
                                                ...
                                                style={{ flex: 1, fontSize: 12, border: "none", borderBottom: "1px solid var(--primary)", background: "transparent", outline: "none", color: "var(--foreground)" }}
                                            />
                                        ) : (
                                            <span style={{ flex: 1, fontSize: 12, fontWeight: conv.id === convId ? 600 : 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--foreground)" }}>
                                                {conv.name}
                                            </span>
                                        )}
                                        <div style={{ display: "flex", gap: 2, flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                                            <button title="Rename" onClick={(e) => startRename(conv, e)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--muted-foreground)", padding: 3, borderRadius: 4, display: "flex" }}>
                                                <PencilIcon />
                                            </button>
                                            <button title="Delete" onClick={(e) => handleDelete(conv.id, e)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--muted-foreground)", padding: 3, borderRadius: 4, display: "flex" }}>
                                                <TrashIcon />
                                            </button>
                                        </div>
                                    </div>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "var(--background)" }}>
                    <div style={{ padding: "12px 24px", borderBottom: "1px solid var(--border)", background: "var(--card)", flexShrink: 0, minHeight: 49, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        {convName ? (
                        ...
                        {draftTemplateId && (
                            <button
                                onClick={() => navigate("/contract-templates", { state: { editId: draftTemplateId } })}
                                style={{
                    ...
                    <div style={{ flex: 1, overflowY: "auto", padding: "24px" }}>
                        {messages.length === 0 && (
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 10, color: "var(--muted-foreground)" }}>
                                <p style={{ fontWeight: 700, fontSize: 20, color: "var(--foreground)" }}>Let's draft a contract.</p>
                                <p style={{ fontSize: 14 }}>Tell me about the parties, scope, and payment terms.</p>
                                <p style={{ fontSize: 12 }}>I'll ask what's missing and put together a reusable template.</p>
                            </div>
                        )}
                    ...
                    <div style={{ padding: "16px 24px", borderTop: "1px solid var(--border)", background: "var(--card)", flexShrink: 0 }}>
                        <div style={{ display: "flex", gap: 10, alignItems: "flex-end", maxWidth: 800, margin: "0 auto" }}>
                            <textarea
                                ...
                                disabled={sending}
                                style={{
                                    flex: 1, resize: "none", border: "1px solid var(--border)", borderRadius: 12,
                                    padding: "10px 14px", fontSize: 14, lineHeight: 1.5, background: "var(--background)",
                                    color: "var(--foreground)", outline: "none", fontFamily: "inherit", maxHeight: 120,
